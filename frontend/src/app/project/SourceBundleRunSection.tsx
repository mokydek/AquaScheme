...
export function SourceBundleRunSection({
  ...
  const onConditions = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Ни одна прочитанная величина в поля расчёта не подставляется — все
      // уходят на сверку.
      //
      // Диаметр из технического обследования — это диаметр СУЩЕСТВУЮЩЕЙ трубы
      // («Керамическая труба Ø450 мм» в разделе характеристик обследуемой
      // сети), а поле рядом просит ПРОЕКТНЫЙ, который назначается техническими
      // условиями. Подстановка выдавала измеренное за проектное решение и
      // противоречила второму источнику: подписи вдоль трассы дают Ø300.
    } catch (cause) {
