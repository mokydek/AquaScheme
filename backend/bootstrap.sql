...
-- ============================================================
-- [bootstrap] ОТКЛЮЧЕНО: datasets_kind_check переписывается ниже. На пустой базе эта редакция безвредна, на базе с данными — роняет применение, а её результат всё равно затирает последняя редакция.
-- [bootstrap] alter table public.datasets drop constraint if exists datasets_kind_check;
-- [bootstrap] alter table public.datasets add constraint datasets_kind_check
-- [bootstrap]   check (kind in (
-- [bootstrap]     'topography', 'buildings', 'source', 'geology', 'seismic', 'normative',
-- [bootstrap]     'equipment', 'region', 'drainage', 'basis', 'route_constraints', 'route_audit'
-- [bootstrap]   ));
...
-- [bootstrap] ОТКЛЮЧЕНО: projects_route_status_check переписывается ниже. На пустой базе эта редакция безвредна, на базе с данными — роняет применение, а её результат всё равно затирает последняя редакция.
-- [bootstrap] alter table public.projects drop constraint if exists projects_route_status_check;
-- [bootstrap] alter table public.projects add constraint projects_route_status_check
-- [bootstrap]   check (route_status in ('stale', 'blocked', 'preliminary', 'calculated'));
...
-- [bootstrap] ОТКЛЮЧЕНО: nodes_kind_check переписывается ниже. На пустой базе эта редакция безвредна, на базе с данными — роняет применение, а её результат всё равно затирает последняя редакция.
-- [bootstrap] alter table public.nodes drop constraint if exists nodes_kind_check;
-- [bootstrap] alter table public.nodes add constraint nodes_kind_check check (kind in (
-- [bootstrap]   'source', 'junction', 'hydrant', 'valve', 'air_valve', 'drain', 'well',
-- [bootstrap]   'ring', 'cross', 'building', 'facility_inflow', 'lns_inlet', 'lns_outlet',
-- [bootstrap]   'outlet', 'manhole', 'terrain_break', 'treatment_facility',
-- [bootstrap]   'pumping_station', 'gravity_inlet', 'pressure_outlet', 'chamber',
-- [bootstrap]   'outfall', 'crossing', 'transition', 'inspection_node'
-- [bootstrap] ));
...
-- ============================================================
-- [bootstrap] ОТКЛЮЧЕНО: datasets_kind_check переписывается ниже. На пустой базе эта редакция безвредна, на базе с данными — роняет применение, а её результат всё равно затирает последняя редакция.
-- [bootstrap] alter table public.datasets drop constraint datasets_kind_check;
-- [bootstrap] alter table public.datasets add constraint datasets_kind_check
-- [bootstrap]   check (kind in ('topography', 'buildings', 'source', 'geology', 'seismic', 'normative', 'equipment', 'region'));
...
-- [bootstrap] alter table public.datasets drop constraint datasets_kind_check;
-- [bootstrap] alter table public.datasets add constraint datasets_kind_check
-- [bootstrap]   check (kind in ('topography', 'buildings', 'source', 'geology', 'seismic', 'normative', 'equipment', 'region', 'drainage'));
...
-- [bootstrap]   check (kind in (
-- [bootstrap]     'topography', 'buildings', 'source', 'geology', 'seismic', 'normative',
-- [bootstrap]     'equipment', 'region', 'drainage', 'basis'
-- [bootstrap]   ));
-- END 0011_basis.sql
...
-- [bootstrap]     'topography', 'buildings', 'source', 'geology', 'seismic', 'normative',
-- [bootstrap]     'equipment', 'region', 'drainage', 'basis', 'route_constraints', 'route_audit',
-- [bootstrap]     'manhole_catalog', 'pump_catalog'
-- [bootstrap]   ));
...
-- [bootstrap]     'topography', 'buildings', 'source', 'geology', 'seismic', 'normative',
-- [bootstrap]     'equipment', 'region', 'drainage', 'basis', 'route_constraints', 'route_audit',
-- [bootstrap]     'manhole_catalog', 'pump_catalog', 'title_block'
-- [bootstrap]   ));
...
-- [bootstrap]     'topography', 'buildings', 'source', 'geology', 'seismic', 'normative',
-- [bootstrap]     'equipment', 'region', 'drainage', 'basis', 'route_constraints', 'route_audit',
-- [bootstrap]     'manhole_catalog', 'pump_catalog', 'title_block', 'master_plan'
-- [bootstrap]   ));
...
-- [bootstrap]     'topography', 'buildings', 'source', 'geology', 'seismic', 'normative',
-- [bootstrap]     'equipment', 'region', 'drainage', 'basis', 'route_constraints', 'route_audit',
-- [bootstrap]     'manhole_catalog', 'pump_catalog', 'title_block', 'master_plan', 'vertical_plan'
-- [bootstrap]   ));
...
-- ============================================================
-- BEGIN 0022_basis_kit_documents.sql
-- ============================================================
...
notify pgrst, 'reload schema';
-- END 0022_basis_kit_documents.sql

-- ============================================================
-- BEGIN 0023_basis_topo_survey.sql
-- ============================================================
...
notify pgrst, 'reload schema';
-- END 0023_basis_topo_survey.sql
