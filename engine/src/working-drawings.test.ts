...
describe('представление профиля при разбивке на бассейны', () => {
  ...
  it('без выбора стоп-фактор с адресом раздела', () => {
    ...
    // Адрес — это раздел, якорь перехода и действие: без якоря подпись остаётся
    // текстом, который ищут прокруткой, а без действия владелец доходит до
    // раздела и снова упирается в «не решено».
    expect(READINESS_SECTIONS.BASIN_PRESENTATION_UNDECIDED).toEqual({
      title: 'Состав проектного комплекта',
      anchor: 'deliverables',
      action: 'Выберите представление профиля: одной лентой или по бассейнам',
    })
  })
