...
/**
 * ФИКСТУРЫ ОБОИХ ОБЪЕКТОВ — дословные фрагменты настоящих отчётов.
 *
 * Не пересказ и не извлечённый текст: дефект «1» и «2» виден ТОЛЬКО в исходном
 * DOCX, потому что `docxText` отдаёт ячейки таблиц через табуляцию, а в
 * `raw/geology_report.txt` табуляций нет и разбор выглядит исправным. Проверка
 * на пересказе доказала бы ровно ничего — на этом дефект и дожил.
 *
 * Обе формы записи здесь нужны вместе: правило, которое чинит одну, обязано не
 * трогать другую, и держать их в одном файле дешевле, чем вспоминать про это.
 */
const TAB = '\t'

/** Станкевича, DOCX: таблица №3 промерзания и таблица 9 трудности разработки. */
const STANKEVICHA_REPORT = [
  'Нормативная глубина промерзания по г. Алматы',
  'Таблица №3',
  `Наименование грунта${TAB}Г. Алматы`,
  ...
  'Глубина нулевой изотермы в грунте: средняя из максимальных – 43см, максимум обеспеченностью 0,90 - 64см.',
  '',
  ...
  'Классификация грунтов по трудности разработки одноковшовым экскаватором приведены в таблице 9.',
  'таблица 9',
  `Наименование и краткая характеристика грунтов${TAB}Для разработки экскаватором${TAB}Для ручной разработки`,
  // ЭТИ ДВЕ СТРОКИ И ЕСТЬ ДЕФЕКТ: «2» и «1» в конце — номера столбцов таблицы
  // трудности разработки, а не метры. Перед ними стоит название грунта,
  // поэтому защита «кандидат без грунта — не кандидат» их не ловит.
  `1${TAB}Суглинок твердый -35в;${TAB}2${TAB}2`,
  `2${TAB}Песок пылеватый, средней крупности – 29а ;${TAB}1${TAB}1`,
].join('\n')

/**
 * Коллектор, PDF: одна строка, единица «см» стоит в ЗАГОЛОВКЕ раздела.
 *
 * Величины записаны голыми числами — 171, 208, 222, 253, — и читаются верно
 * только по порядку величины. Требование «единица рядом с числом» убило бы их
 * все: это ловушка, а не строгость.
 */
const COLLECTOR_REPORT = 'Нормативная глубина сезонного промерзания грунтов, см'
  + '  (СНиП РК 5.01 -102-2013 , СНиП РК 2.04 -01-2017):'
  + '  -   суглинки и глины   - 171;  -   супеси, пески мелкие и пылеватые   - 208;'
  + '  -   пески средние,   крупные и гравелистые   - 222;  -   крупнообломочные грунты   - 253.'
  + '  Среднегодовое количество осадков   - 319   мм,  в том числе в холодный период   - 99   мм.'
  + '  Толщина снежного покрова с 5% вероятностью превышения   -   39 см.'
  + '  Количество дней: с градом   - 2;  с гололёдом   - 6;  с туманами   - 23;  с метелями   - 26;'

describe('кандидатом становится только то, что записано как глубина', () => {
  it('Станкевича: четыре кандидата, номеров столбцов среди них нет', () => {
    const summary = parseGeologyReportSummary(STANKEVICHA_REPORT)
    expect(summary.freezingDepthCandidates.map((item) => item.valueM))
      .toEqual([0.79, 0.96, 1.03, 1.17])
    expect(summary.freezingDepthCandidates.map((item) => item.soil)).toEqual([
      'Суглинок, глина',
      'Супеси, песок мелкий, песок пылеватый',
      'Песок средней крупности, крупный, гравелистый',
      'Крупнообломочные',
    ])
    // 1,17 подтверждён только таблицей — прозы про крупнообломочные в отчёте
    // нет. Отфильтровать по `confirmedByBothForms` значило бы выбросить
    // настоящего кандидата; проверка стоит здесь, чтобы это не соблазняло.
    expect(summary.freezingDepthCandidates.map((item) => item.confirmedByBothForms))
      .toEqual([true, true, true, false])
  })

  it('Станкевича: отброшенные строки названы, а не выброшены молча', () => {
    const summary = parseGeologyReportSummary(STANKEVICHA_REPORT)
    // Молчаливая потеря кандидата не лучше молчаливой выдумки: строка, про
    // которую разбор не может решить, показывается отдельно и с цитатой.
    expect(summary.freezingDepthUnitlessRows.map((row) => [row.raw, row.quote])).toEqual([
      [2, `1${TAB}Суглинок твердый -35в;${TAB}2${TAB}2`],
      [1, `2${TAB}Песок пылеватый, средней крупности – 29а ;${TAB}1${TAB}1`],
    ])
  })

  it('коллектор: голые числа без единицы читаются как прежде', () => {
    const summary = parseGeologyReportSummary(COLLECTOR_REPORT)
    expect(summary.freezingDepthCandidates.map((item) => [item.valueM, item.readAs])).toEqual([
      [1.71, 'cm'], [2.08, 'cm'], [2.22, 'cm'], [2.53, 'cm'],
    ])
    expect(summary.freezingDepthCandidates.map((item) => item.soil)).toEqual([
      'суглинки и глины',
