...
const PROFILE_SIDE_ALLOWANCE_BASE_MM = 3.2
...
function buildAlbumManifest(
  ...
  const generatedPageFormat = (sheet: WorkingDrawingSheet): WorkingDrawingAlbumPage['pageFormat'] => {
    ...
    const fixedAllowanceMm = sheet.kind === 'plan'
      ? 220
      : Math.ceil(heightMm * PROFILE_SIDE_ALLOWANCE_SHARE + PROFILE_SIDE_ALLOWANCE_BASE_MM)
