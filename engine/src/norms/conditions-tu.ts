 ...
 *
 * Распознавание портит буквы вокруг числа чаще, чем само число: цифры крупные
 * и различимые, а «Д», «мм», «⌀» слипаются, теряются и подменяются. Шаблон
 * «Д=» на такой строке не срабатывает — и верно прочитанное 450 не доходит до
 * инженера ВОВСЕ. Молча потерянная находка это та же тихая потеря данных,
 * только на новом уровне.
 *
 * Оговорка о том, чего здесь НЕ измерено: «0=450 00» с синтетического скана
 * получалось из-за шрифта оснастки без кириллицы, а не из-за распознавания. С
 * нормальным начертанием «Д=450 мм» читается дословно. Насколько часто якорь
 * рушится на НАСТОЯЩИХ сканах — покажут документы владельца.
 *
