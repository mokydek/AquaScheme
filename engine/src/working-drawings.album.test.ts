...
describe('высота листа профиля считается, а не назначается', () => {
  ...
  it('высота рулонного листа постоянна независимо от перепада', () => {
    // Правило оформления взято у эталонного альбома: его собственный штамп
    // называет формат «297х2200», и все листы планов и профилей имеют высоту
    // 297 мм — меняется только длина. Прежде наш лист рос вместе с перепадом
    // и доходил до 2863 пт при 842 у эталона; ни один формат не совпадал.
    for (const spanM of [3, 12, 24, 48]) {
      expect(sheetHeights(spanM), `перепад ${spanM} м`).toEqual(sheetHeights(3))
      expect(sheetHeights(spanM).every((height) => height === 297), `перепад ${spanM} м`).toBe(true)
    }
  })

  it('перепад, не помещающийся в полосу, не делает лист выше', () => {
    // Он решается сменой условного горизонта ВНУТРИ листа — это забота
    // отрисовки, а не формата страницы.
    expect(sheetHeights(48).every((height) => height === 297)).toBe(true)
  })
})
