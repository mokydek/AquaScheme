...
export function buildReconstructionFromSurvey(
  ...
  // Расчётный расход из съёмки не выводится и выводиться не может: уличный
  // коллектор собирает сток с бассейна за границами чертежа, а зданий с
  // площадями и этажностью в нём нет. Величина приходит из технических условий
  // владельца сети.
  //
  // Сказать об этом обязан и сам модуль. Прежде о нуле расхода сообщал только
  // гидравлический решатель через `noDesignFlow`, а список стоп-факторов
  // реконструкции молчал — и выглядело это так, будто гидравлика посчитана.
  blockers.push('Расчётный расход не задан: гидравлика не считается, диаметр по расходу '
    + 'не подбирается и скорости не проверяются. Из съёмки расход не выводится — '
    + 'уличный коллектор собирает сток с бассейна за границами чертежа. '
    + 'Величина берётся из технических условий владельца сети.')
